...
from fastapi_plugin.test_utils import generate_token, generate_dpop_proof, generate_dpop_bound_token, PUBLIC_DPOP_JWK 
...
def test_auth0fastapi_accepts_dpop_configuration():
    """Test that Auth0FastAPI accepts DPoP configuration parameters."""
    auth0 = Auth0FastAPI(
        domain="auth0.local",
        audience="test_audience",
        dpop_enabled=True,
    ...
    options = auth0.api_client.options
    assert options.dpop_enabled == True
    assert options.dpop_required == True
    assert options.dpop_iat_leeway == 60
    assert options.dpop_iat_offset == 600

@pytest.mark.asyncio
async def test_should_return_200_with_dpop_authentication(httpx_mock: HTTPXMock):
    """
    Test successful DPoP authentication with proper DPoP proof and bound access token.
    """
    # Mock OIDC discovery and JWKS with both RSA and EC keys
    httpx_mock.add_response(
        method="GET",
        url="https://auth0.local/.well-known/openid-configuration",
        json={
    ...
    # Generate DPoP-bound access token
